...
const TYPES = {
  ...
  py: 'int float str bool list dict set tuple None range enumerate zip map filter open print len type object Exception str bytes frozenset complex property dataclass Enum Counter defaultdict deque Path datetime json re asyncio np pd DataFrame Series Index MultiIndex RangeIndex DatetimeIndex PeriodIndex Categorical Timestamp array zeros ones full empty arange linspace logspace geomspace meshgrid eye identity diag fromfunction fromiter fromstring frombuffer fromfile loadtxt genfromtxt memmap savetxt savez reshape ravel flatten transpose T newaxis squeeze expand_dims broadcast_to broadcast_arrays dot matmul inner outer tensordot einsum linalg inv det eig eigh eigvals svd qr lstsq norm matrix_power kron concatenate stack vstack hstack split array_split tile repeat copy where nonzero argwhere argmin argmax take put compress digitize allclose isclose isnan isinf isfinite nan_to_num nansum nanmean nanstd nanprod nanmin nanmax alltrue sometrue in1d intersect1d setdiff1d union1d cumsum cumprod diff trapezoid trapz product round floor ceil clip percentile median var std bincount histogram random binomial poisson exponential gamma beta chisquare laplace logistic rayleigh standard_normal uniform randint random_sample seed SeedSequence default_rng spawn asarray ascontiguousarray asfortranarray ix_ s_ r_ mgrid ogrid applymap map transform pipe assign query eval str.contains str.startswith str.endswith str.upper str.lower str.strip str.split str.join str.replace str.extract str.match str.findall str.len dt.year dt.month dt.day dt.hour dt.minute dt.second dt.day_name dt.weekday dt.tz dt.tz_localize dt.tz_convert rolling expanding resample shift diff pct_change groupby agg aggregate filter merge join concat append to_csv to_excel to_json to_parquet to_pickle to_sql to_dict to_numpy to_markdown to_html to_latex read_csv read_excel read_json read_parquet read_pickle read_sql read_sql_query read_table set_index reset_index rename drop dropna fillna ffill bfill isnull notnull astype convert_dtypes to_numeric to_datetime melt pivot pivot_table crosstab stack unstack explode nlargest nsmallest value_counts sort_values sort_index info describe head tail sample iterrows itertuples items iat at loc iloc xs',
