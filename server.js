...
// NumPy 教程
const numpyFeatures = require('./data/numpyFeatures');
// Pandas 教程
...
// ---------- NumPy 教程 ----------
app.get('/api/numpy', (req, res) => {
  const { q } = req.query;
  let list = numpyFeatures;
  if (q) {
    const kw = String(q).toLowerCase();
    list = list.filter(f =>
...
app.get('/api/numpy/:id', (req, res) => {
  const f = numpyFeatures.find(x => x.id === req.params.id);
  if (!f) return res.status(404).json({ error: '特性不存在' });
  res.json(f);
});
...
// ---------- 网络通信协议（Protobuf + MQTT + WebSocket） ----------
app.get('/api/protocol', (req, res) => {
  const { q } = req.query;
  let list = protocolFeatures;
  if (q) {
    const kw = String(q).toLowerCase();
    list = list.filter(f =>
      (f.title + f.summary + f.detail.join(' ') + f.example + (f.example2 || '') + (f.example3 || '')).toLowerCase().includes(kw));
